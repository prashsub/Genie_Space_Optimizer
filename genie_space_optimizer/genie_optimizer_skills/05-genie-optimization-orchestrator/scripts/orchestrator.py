...
def _setup_worker_imports(worker_dir: str | None = None):
    ...
    # parent.parent.parent reaches the semantic-layer/ directory where
    # genie-optimization-workers/ lives as a sibling of 05-genie-optimization-orchestrator/
    _sem_layer = Path(__file__).resolve().parent.parent.parent

    candidate_dirs = [
        worker_dir,
        str(_sem_layer / "genie-optimization-workers" / "03-genie-metadata-optimizer" / "scripts"),
    ]
    optimizer_dir = None
    ...
    applier_candidates = [
        worker_dir,
        str(_sem_layer / "genie-optimization-workers" / "04-genie-optimization-applier" / "scripts"),
    ]
    ...
    generator_candidates = [
        worker_dir,
        str(_sem_layer / "genie-optimization-workers" / "01-genie-benchmark-generator" / "scripts"),
    ]
...
def _fetch_space_config(space_id: str) -> dict:
    ...
    print(f"  Space state: tables={len(tables_list)}, metric_views={len(mvs_list)}, "
          f"tvfs={len(funcs_list)}, instructions={'present' if has_instructions else 'absent'}")
    return config
...
def trigger_evaluation_job(
    ...
    if result.returncode != 0:
        print(f"  TRIGGER FAILED: {result.stderr[:500]}")
    ...
    if not run_id:
        print(f"  WARNING: Could not parse run_id from output:\n{combined[:300]}")
    return {"status": "TRIGGERED", "run_id": run_id, "stdout": combined}
...
def deploy_bundle_and_run_genie_job(target="dev", genie_job="genie_spaces_deployment_job"):
    ...
    if genie_job:
        ...
        run = subprocess.run(
            ["databricks", "bundle", "run", "-t", target, genie_job, "--no-wait"],
            capture_output=True, text=True, timeout=120,
        )
        if run.returncode != 0:
            err = run.stderr
            if "not found" in err.lower():
                print(f"  NOTE: Job '{genie_job}' not defined in bundle. Skipping.")
            else:
                return {"status": "JOB_FAILED", "error": err}
        else:
            print("  Genie Space deployment job triggered.")
